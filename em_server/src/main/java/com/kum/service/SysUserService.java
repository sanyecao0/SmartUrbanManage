...
public class SysUserService {
     ...
     * @return 返回用户ID
     */
    public String register(SysUser sysUser, boolean isAdmin) {
        sysUser.setPassword(
                bCryptPasswordEncoder.encode(sysUser.getPassword()));
        save(sysUser);
        String userId = loadUserByUsername(sysUser.getUserName()).getId();

        int targetRole = isAdmin ? 0 : 1;
        sysUserRoleService.save(new SysUserRole().toBuilder().userId(userId).roleId(targetRole).build());
        return userId;
