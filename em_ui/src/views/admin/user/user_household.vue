<template>
  <page-main>
    <a-card :loading="loading" title="用户信息管理">
      <div class="head">
        <span>搜索类型</span>
        <a-select
          size="large"
          default-value="用户信息名称"
  ...
  methods: {
    Get_householdDataList () {
      getHousehold().then(res => {
        ...
        this.household_save_title = '新增用户信息'
      })
    },
    Get_users () {
      getUsers().then(res => {
    ...
    Edit_householdData (form) {
      this.household_save_title = '编辑用户信息'
