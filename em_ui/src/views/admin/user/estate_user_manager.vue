<template>
  <page-main>
    <a-card :loading="loading" title="城市管理人员管理">
      <div class="head">
        <span>搜索类型</span>
        ...
        <a-button type="primary" style="height: 40px;" @click="estateUser_save_modalVisible = true">添加管理人员</a-button>
        <a-button type="danger" v-if="table_selectedRowKeys.length > 0" style="height: 40px; margin-left: 10px;"
          @click="Del_batchData">删除被选择的「管理人员」</a-button>
      </div>
  ...
  methods: {
    Get_estateUserDataList() {
      getEstateUser().then(res => {
        ...
        this.estateUser_save_title = '新增工作人员'
      })
    },
    ...
    Edit_estateUserData(form) {
      this.estateUser_save_title = '编辑工作人员'
