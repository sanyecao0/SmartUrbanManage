<template>
  <page-main>
    <a-card :loading="loading" title="缴费类型管理">
      <div class="head">
        <span>搜索类型</span>
        <a-select
          size="large"
          default-value="缴费类型名称"
          v-model="chargeType_query_type"
          style="width: 200px;"
        >
          <a-select-option value="chargeName">缴费类型名称</a-select-option>
        </a-select>
        <a-input-search
      ...
      >
        ...
        <a-table-column key="chargeName" title="缴费类型名称" data-index="chargeName" />
        <a-table-column key="chargeMoney" title="缴费金额(元)" data-index="chargeMoney" />
    ...
    >
      ...
      >
        <a-form-model-item label="缴费名称" prop="chargeName">
          <a-input v-model="chargeType_form_data.chargeName" />
        </a-form-model-item>
        <a-form-model-item label="缴费金额(每月)" prop="chargeName">
  ...
  methods: {
    Get_chargeTypeDataList () {
      getChargeType().then(res => {
        ...
        this.chargeType_save_title = '新增缴费类型'
      })
    },
    ...
    Edit_chargeTypeData (form) {
      this.chargeType_save_title = '编辑缴费类型'
